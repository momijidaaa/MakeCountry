...
world.afterEvents.itemUse.subscribe((ev)=>{
    switch(ev.itemStack.typeId){
        case "karo:countryinvite":{
            ...
            addScore(`${ev.itemStack.getLore()[0]}`,`mc_people`,1)
...
export function countryInfo(source , name) {
  const all_score = world.scoreboard.getObjective(`mc_chunk`).getScores();
  const chunks = all_score.filter(nameScore => nameScore.score === getNumberScore(`mc_countries`,name));
  const form = new UI.ActionFormData()
  let peace = `§cNo`
  if(isPeaceCountry(getNameScore(`mc_countries`,getNumberScore(`mc_countries`,name)))) peace = `§aYes`
  form.title(name)
  form.body(`国名: ${getNameScore(`mc_countries`,getNumberScore(`mc_countries`,name))} §r\n所有チャンク数: ${chunks.length}チャンク\n国民数: ${getScore(`${getNumberScore(`mc_countries`,name)}`,`mc_people`)}人\n平和主義: ${peace}§r\n国庫金: ${getScore(`${getNumberScore(`mc_countries`,name)}`,`countrymoney`)}円\n税徴収率: ${getScore(`${getNumberScore(`mc_countries`,name)}`,`countrymoneyper`)}パーセント\n維持費支払い回数: ${getScore(`${getNumberScore(`mc_countries`,name)}`,`mc_days`)}回\n赤字連続回数: ${getScore(`${getNumberScore(`mc_countries`,name)}`,`mc_notax`)}回`)
  form.button(`§lok`)
  form.show(source).then((rs)=>{})
}

export function countryList(source) {
  const all_score = world.scoreboard.getObjective(`mc_countries`).getScores().filter(kuni => kuni.score > 0);
  const form = new UI.ActionFormData()
  for(const c of all_score) {
    form.button(`${c.participant.displayName}`)
  }
  form.title(`情報を確認したい国を選んでください`)
  if(all_score.length === 0) {
    NotCountryIs(source)
    return
  }
  form.show(source).then((rs)=>{
    if(rs.canceled) return
    countryInfo(source,all_score[rs.selection].participant.displayName)
  })
}
...
export function countryCreate(source, countryName) {
    ...
    if(getScore(convertChunk(source.location.x,source.location.z),`mc_chunk`) === -1) {
...
export function countryDelete(source) {
    ...
    world.scoreboard.getObjective(`mc_notax`).removeParticipant(`${sourcecountry}`)
    world.scoreboard.getObjective(`mc_core`).removeParticipant(`${sourcecountry}`)
    world.scoreboard.getObjective(`mc_days`).removeParticipant(`${sourcecountry}`)
    world.scoreboard.getObjective(`mc_people`).removeParticipant(`${sourcecountry}`)
...
export function countryAddMember(source){
    ...
    form.show(source).then((rs)=>{
      ...
      if(players[rs.formValues[0]].getComponent(`inventory`).container.emptySlotsCount === 0) {
        source.sendMessage(`§c相手の持ち物がいっぱいで招待出来ません`)
        return
      }
