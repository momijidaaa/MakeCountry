...
export const configs = {
    ...
    //所属国以外の国での動作の設定
    noPlaceInCountry: false , //所属国以外の国にブロックを置けないようにするか
    noBreakInCountry: false , //所属国以外の国でブロックを壊せないようにするか
    ...
    noPlaceInSpecialZone: false , //特別区域(管理者が設定可能)にブロックを置けないようにするか
    noBreakInSpecialZone: false , //特別区域(管理者が設定可能)でブロックを壊せないようにするか
    noExplodeInSpecialZone: false , //特別区域(管理者が設定可能)で爆発物が爆発しないようにするか

    //建国関係の設定
    makeCountryCost: 10000, //建国にかかるお金
    buyChunkCost: 3000, //領土の追加購入にかかるお金(1チャンク)
    sellChunk: 2000, //購入したチャンクを売るともらえる金額(1チャンク)
    firstPeace: true, //建国時に平和主義として設定するかどうか

    //金稼ぎ系

    CurrencyUnit: `円`, //ゲーム内通貨の単位
    ...
    coreName: `§d国のコア`, //コアの名前
    noExplodeInCountry: false, //各国の領土内で爆発物が爆発しないようにするか
    taxTime: 1440, //税金を何分ごとに徴収するか(現実時間) デフォルト値(1440分(24時間))
    taxFreeTime: 3, //建国から何回目の徴収まで免税にするか(デフォルトでは3)
    taxPerOfPeopleCount: 0.01, //国民1人あたりの維持費の増加(維持費+(維持費×国民数×〇))
    taxAmount: 5, //通常の国にかかる維持費(1チャンクあたり)
    taxPeaceAmount: 50, //平和主義の場合に追加でかかる維持費(1チャンクあたり)
    deleteCountryReasonNoMoney: 10, //何回連続で徴収時に赤字になったら国を自動的に消すか
