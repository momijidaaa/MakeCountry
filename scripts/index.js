...
world.events.on("tick", (ev) => {
    ...
    for (const player of world.getPlayers()) {
        const startId = 10000, players = MainDB.get("players");
        const distId = player.getTags().find(tag => tag.startsWith("distID:"))?.replace("distID:", "");

        if (distId) {
            player.distId = distId;
            if (!players.has(distId)) players.setup(player);
        }
        else {
            player.addTag(`distID:${startId + players.size}`);
            player.distId = String(startId + players.size);
            players.setup(player);
        }
